...
namespace CodeStage.Maintainer.Cleaner
{
	...
	public static class ProjectCleaner
	{
		...
		public static CleanerRecord[] StartSearch(bool showResults)
		{
			...
			if (searchCanceled)
			{
				Debug.Log(Maintainer.ConstructLog("Search canceled by user!"));
				return null;
			}
			...
			try
			{
				...
				if (!searchCanceled)
				{
					Debug.Log(Maintainer.ConstructLog("Results: " + results.Count +
		...
		{
			if (!ProjectSettings.Cleaner.findEmptyFolders ||
				!ProjectSettings.Cleaner.findEmptyFoldersAutomatically) return;
			...
			if (results.Count > 0)
			{
				...
				if (result == 0)
				{
					...
					Debug.Log(Maintainer.ConstructLog(results.Count + " empty folders cleaned.", ModuleName));
				}
				else if (result == 2)
				{
					SearchResultsStorage.CleanerSearchResults = results.ToArray();
		...
		private static bool ScanFolders(ICollection<CleanerRecord> results, bool showProgress = true)
		{
			...
			if (showProgress)
			{
				EditorUtility.DisplayProgressBar(
					string.Format(ProgressCaption, currentPhase, phasesCount, folderIndex, foldersCount),
					"Getting all folders...", (float)folderIndex / foldersCount);
			}
		...
		private static bool ScanProjectFiles(ICollection<CleanerRecord> results, bool showProgress = true)
		{
			...
			if (ignoredScenes.Count == 0)
			{
				...
				results.Add(CleanerWarningRecord.Create(
					"No scenes added to the build settings or Filters > Scenes Ignores tab.\n" +
			...
			{
				editorIgnoresFilters.Clear();
				editorIgnoresFilters.AddRange(ProjectCleanerSettings.GetEditorFilters()); 
				...
				for (var i = 0; i < count; i++)
				{
					...
					if (!AssemblyDefinitionParser.IsEditorOnlyAssembly(asset.Path))
						continue;

					var assemblyDirectory = CSPathTools.EnforceSlashes(Path.GetDirectoryName(asset.Path));
					editorIgnoresFilters.Add(FilterItem.Create(assemblyDirectory, FilterKind.Directory));
				}
			}
			...
			for (var i = count - 1; i > -1; i--)
			{
				    ...
				    && i != 0)
				{
					...
					if (EditorUtility.DisplayCancelableProgressBar(
							string.Format(ProgressCaption, currentPhase, phasesCount, index, count),
							"Populating results...",
							(float)index / count))
					{
						return true;
		...
		private static bool AssetInIgnores(AssetInfo assetInfo, List<string> ignoredScenes)
		{
			...
			if (assetInfo.Type == CSReflectionTools.spriteAtlasType)
			{
				...
				if (atlas != null)
				{
					...
					{
						Debug.LogError(Maintainer.ErrorForSupport("Can't parse UnityEngine.U2D.SpriteAtlas!", ModuleName));
					}
				}
				else
				{
					Debug.LogWarning(Maintainer.ErrorForSupport("Couldn't load SpriteAtlas: " + assetInfo.Path, ModuleName));
				}
			}
			...
			{
				var referencedAtAsset = referencedAtInfo.assetInfo;
				if (referencedAtAsset.SettingsKind != AssetSettingsKind.Undefined && 
		...
		private static void CheckScenesForExistence(ICollection<CleanerRecord> results, List<string> ignoredScenes)
		{
			for (var i = ignoredScenes.Count - 1; i >= 0; i--)
			{
				...
				{
					results.Add(CleanerErrorRecord.Create("Scene " + Path.GetFileName(scenePath) +
														  " from Ignores or Build Settings not found!"));
