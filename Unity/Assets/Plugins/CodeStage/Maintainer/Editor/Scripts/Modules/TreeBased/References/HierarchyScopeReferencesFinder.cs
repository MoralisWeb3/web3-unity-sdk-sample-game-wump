...
namespace CodeStage.Maintainer.References
{
	...
	internal static class HierarchyScopeReferencesFinder
	{
		...
		public static HierarchyReferenceItem[] FindHierarchyObjectsReferences(Object[] allObjects, Object[] newObjects, bool showResults = true)
		{
			...
			try
			{
				...
				if (!searchCanceled)
				{
					...
					Debug.Log(Maintainer.ConstructLog("Results: " + referencesFound +
													  " references found in " + sw.Elapsed.TotalSeconds.ToString("0.000", CultureInfo.InvariantCulture) +
													  " seconds.", ReferencesFinder.ModuleName));
				...
				{
					Debug.Log(Maintainer.ConstructLog("Search canceled by user!", ReferencesFinder.ModuleName));
				}
			}
			catch (Exception e)
			{
				Maintainer.PrintExceptionForSupport("Something went wrong while finding references!", ReferencesFinder.ModuleName, e);
				EditorUtility.ClearProgressBar();
			}
		...
		private static ExactReferenceData ObjectToReferencingEntry(Object target)
		{
            ...
            {
	            Debug.LogError(Maintainer.ErrorForSupport("Couldn't find referenced Game Object from object " + target));
