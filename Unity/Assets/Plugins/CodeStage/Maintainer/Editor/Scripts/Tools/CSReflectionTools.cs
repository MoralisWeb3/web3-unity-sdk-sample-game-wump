...
/*#if UNITY_2018_2_OR_NEWER
		...
		public static void SetInspectorToDebug(SerializedObject serializedObject)
		{
			if (inspectorModeCachedSetter == null)
			{
				...
				if (pi != null)
				{
					...
					{
						Debug.LogError(Maintainer.ErrorForSupport("Can't get the setter for the SerializedObject.inspectorMode property!"));
						return;
					}
				}
				else
				{
					Debug.LogError(Maintainer.ErrorForSupport("Can't get the SerializedObject.inspectorMode property!"));
					return;
				}
			}
		...
		public static void BeginVerticalSplit(object spl, GUILayoutOption[] guiLayoutOptions)
		{
			...
			if (beginVerticalSplitMethodInfo == null)
			{
				Debug.LogError(Maintainer.ErrorForSupport("Can't get SplitterGUILayout.BeginVerticalSplit method!"));
				return;
			}
		...
		public static void EndVerticalSplit()
		{
			...
			if (endVerticalSplitMethodInfo == null)
			{
				Debug.LogError(Maintainer.ErrorForSupport("Can't get SplitterGUILayout.EndVerticalSplit method!"));
				return;
			}
		...
		public static UnityEventBase GetDummyEvent(SerializedProperty property)
		{
			...
			if (getDummyEventMethodInfo == null)
			{
				Debug.LogError(Maintainer.ErrorForSupport("Can't get UnityEventDrawer.GetDummyEvent method!"));
				return null;
			}
		...
		public static bool IsPropertyIsSubclassOf<T>(SerializedProperty property)
		{
		...
		public static bool IsPropertyIsSubclassOf(SerializedProperty property, Type type)
		{
			...
			Type realType;
			GetFieldInfoFromProperty(property, out realType);

			if (realType == null)
				return false;
			
			var isEventType = realType.IsSubclassOf(type);
			return isEventType;
		}
		...
		public static FieldInfo GetFieldInfoFromProperty(SerializedProperty property, out Type type)
		{
			...
			if (getFieldInfoFromPropertyMethodInfo == null)
			{
				var scriptAttributeUtilityType = EditorWindowType.Assembly.GetType("UnityEditor.ScriptAttributeUtility", false);
				if (scriptAttributeUtilityType == null)
				{
					Debug.LogError(Maintainer.ErrorForSupport("Can't find UnityEditor.ScriptAttributeUtility type!"));
					return null;
				}

				getFieldInfoFromPropertyMethodInfo = scriptAttributeUtilityType.GetMethod("GetFieldInfoFromProperty", BindingFlags.NonPublic | BindingFlags.Static);
			}
			...
			var result = (FieldInfo)getFieldInfoFromPropertyMethodInfo.Invoke(null, parameters);
			var outResult = parameters[1];
